...
# # file = open(r'C:\Users\hp\Desktop\mock final\mockinterview\mockinterview\interview\skill.csv',encoding='utf-8')
# file = open(r'C:\Users\KIRAN\Desktop\vs_django\thejus\skill.csv',encoding='utf-8')
...
# Define 'num' to handle digits
num = set("0123456789")  # This will define the 'num' variable

# Open the CSV file and read the content
file = open(r"C:\Users\msi vishnu\Downloads\placify-demo\placify-demo\Book1.csv", encoding='utf-8')
csvreader = csv.reader(file)
header = next(csvreader)
rows = []
i = 0
...
for i in rows:
    ...
    # Filtering stopwords and stemming words
    for w in word_tokens:
        if w not in stop_words:
            lis = []
            my_string = w
...
model = SVC(kernel='linear')  # You can choose different kernels (e.g., 'linear', 'rbf')
model.fit(X_train, y_train)
...
print(f"Accuracy on Training Data: {score_train}")
...
def predict(txt):
    ...
    res = []
    for val in y2:
        res.append(dclass[val])
    return res

# Example of prediction
txt_example = "area interest deep learn control system design program python"
print(predict(txt_example))
